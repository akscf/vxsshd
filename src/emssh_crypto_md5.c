...
#define MD5_DIGEST_LENGTH  16    /* 128 bit */
#define MD5_BLOCK_LENGTH   64    /* 512 bit */
...
// ----------------------------------------------------------------------------------------------------------
static void md5_process(MD5_CTX *pms, const md5_byte_t *data /*[64]*/) {
	md5_word_t a = pms->abcd[0], b = pms->abcd[1];
	md5_word_t c = pms->abcd[2], d = pms->abcd[3];
	md5_word_t t;
...
static void md5_reset(MD5_CTX *pms) {
...
static void md5_init(MD5_CTX *pms) {
	md5_reset(pms);
}

static void md5_append(MD5_CTX *pms, const md5_byte_t *data, int nbytes) {
...
static void md5_finish(MD5_CTX *pms, md5_byte_t digest[MD5_DIGEST_LENGTH]) {
...
int em_ssh_md5_init(em_ssh_md5_ctx_t **ctx) {
    em_ssh_md5_ctx_t *md_ctx = NULL;

    if (!ctx) {
        return EINVAL;
    }
    md_ctx = em_ssh_mem_zalloc(sizeof(em_ssh_md5_ctx_t), NULL);
    if(md_ctx == NULL) {
        return ENOMEM;
    }
    md5_init(md_ctx);
...
int em_ssh_md5_update(em_ssh_md5_ctx_t *ctx, const void *data, size_t data_size) {
    ...
    md5_append(ctx, data, data_size);
    return OK;
}

int em_ssh_md5_final(em_ssh_md5_ctx_t *ctx, uint8_t *digest) {
    if (!ctx || !digest) {
        return EINVAL;
    }
...
int em_ssh_md5_digest(const void *input, size_t input_len, uint8_t *digest, size_t digest_len) {
    ...
    md5_init(&state);
    md5_append(&state, input, (int) input_len);
