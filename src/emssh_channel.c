...
static void mem_destructor_em_ssh_channel_t(void *data) {
    em_ssh_channel_t *channel = data;
...
static void shell_exit_handler(em_ssh_channel_t *channel) {
    if(!channel) {
        return;
    }
    em_ssh_channel_close(channel);
}
...
int em_ssh_channel_alloc(em_ssh_channel_t **channel, uint32_t chid, uint32_t win_size, uint32_t packet_size) {
    int err = OK;
    em_ssh_channel_t *tch = NULL;

    if(!channel) {
        return EINVAL;
    }

    if((tch = em_ssh_mem_zalloc(sizeof(em_ssh_channel_t), mem_destructor_em_ssh_channel_t)) == NULL) {
        err = ENOMEM;
        goto out;
    }
    tch->id = chid;
...
int em_ssh_channel_start_shell(em_ssh_channel_t *channel) {
    ...
    if((taskNameToId("tShell")) == ERROR) {
        em_ssh_log_warn("tShell not started");
        return ERROR;
    }
    if(!shellLock(TRUE)) {
        em_ssh_log_warn("The shell is currently in use");
        return ERROR;
    }
